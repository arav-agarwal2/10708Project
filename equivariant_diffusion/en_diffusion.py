...
class EnVariationalDiffusion(torch.nn.Module):
    ...
    def logistic_integrate(self,npoints, loc=0.2, scale=0.3, clip=4., device='cuda', deterministic=False):
    ...
    def compute_loss(self, x, h, node_mask, edge_mask, context, t0_always):
        ...
        # Instead of sampling some time-step t, we sample a logsnr value:
        logsnr, weights = self.logistic_integrate(x.shape[0])
        ...
        logsnr2 = logsnr.reshape((bs,1,1)).broadcast_to((bs,nm,1))


        z_t = torch.sqrt(torch.sigmoid(logsnr2)) * xh + torch.sqrt(torch.sigmoid(-logsnr2)) * eps
        ...
        net_out = self.phi(z_t, self.logsnr2t(logsnr), node_mask, edge_mask, context)
        error = sum_except_batch((eps - net_out) ** 2) / denom
        
        # MSE Gap
        error = error - 9.0*torch.sigmoid(logsnr)
        loss = 0.5 * 9.0 * math.log(2 * math.pi * math.e) + 0.5*(error * weights)
